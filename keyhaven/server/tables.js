const accounts = 'CREATE TABLE IF NOT EXISTS accounts (id SERIAL NOT NULL PRIMARY KEY, first_name TEXT, last_name TEXT, email TEXT NOT NULL UNIQUE, password TEXT NOT NULL, profile_image_url TEXT, joined TIMESTAMP(0) NOT NULL, is_verified BOOLEAN NOT NULL);'
...
const passwordSecrets = 'CREATE TABLE IF NOT EXISTS password_secrets (email TEXT PRIMARY KEY, secret_key TEXT, iv TEXT, FOREIGN KEY (email) REFERENCES accounts(email));';
